...
async function pruneOldHistory(nowTs = Date.now()) {
  if (!historyDb) return;
  const cutoff = nowTs - DAY_WINDOW_MS;
...
function updateNodeHistory(node, probe, nowTs = Date.now()) {
  ...
  while (history.length && nowTs - history[0].ts > DAY_WINDOW_MS) history.shift();
...
function computeNodeView(nodeEntry) {
  const samples = nodeEntry.history || [];
  const total = samples.length;
  const upCount = samples.filter((s) => s.online).length;
  ...
  const now = Date.now(), windowStart = now - DAY_WINDOW_MS, bucketSizeMs = DAY_WINDOW_MS / UPTIME_BUCKETS;
  const buckets = Array.from({ length: UPTIME_BUCKETS }, () => ({ total: 0, up: 0, down: 0, maintenance: 0 }));
  for (const sample of samples) {
    if (!sample.ts || sample.ts < windowStart) continue;
    const idx = Math.max(0, Math.min(UPTIME_BUCKETS - 1, Math.floor((sample.ts - windowStart) / bucketSizeMs)));
    const b = buckets[idx];
    b.total++;
    if (sample.online) b.up++; else b.down++;
    if (sample.maintenance) b.maintenance++;
  }
  ...
  const uptimeBars = buckets.map((b) => {
    ...
    const maintenanceDominant = b.maintenance > 0 && b.maintenance >= b.total / 2;
    let level = "mixed";
    if (maintenanceDominant) level = "maint";
    else if (downRatio === 0) level = "up";
    else if (downRatio === 1) level = "down";
    return { level, uptime, downRatio, label: `${Math.round(uptime * 100)}% up / ${Math.round(downRatio * 100)}% down` };
  });
  ...
  const recentPings = samples.slice(-30).filter((s) => s.online && Number.isFinite(s.latencyMs)).map((s) => s.latencyMs);
...
function renderPingGraph(history) {
  if (!history.length) return '<div class="muted small">No ping data yet.</div>';
  const W = 760, H = 110, P = 12;
  const latencies = history.filter((s) => s.online && Number.isFinite(s.latencyMs)).map((s) => s.latencyMs);
  ...
  const points = history.map((s, i) => {
    const x = P + i * stepX;
    if (!s.online || !Number.isFinite(s.latencyMs)) return { x, y: null };
  ...
  for (let i = 0; i < history.length; i++) {
    if (!history[i].online) { if (outStart == null) outStart = i; }
  ...
  const oRects = bands.map((b) => {
    ...
    const tip = escapeHtml(`${history[b.s].at} → ${history[b.e].at} · down ${formatDuration((b.e - b.s + 1) * SAMPLE_INTERVAL_MS)}`);
    return `<rect x="${sx.toFixed(1)}" y="${P}" width="${Math.max(2, ex - sx).toFixed(1)}" height="${uh}" class="outage-band"><title>${tip}</title></rect>`;
  }).join("");

  const pingDots = points.filter((p) => p.y != null)
  ...
  const hits = history.map((s, i) => {
    ...
    const tip = escapeHtml(s.online && Number.isFinite(s.latencyMs) ? `${s.at} · ${s.latencyMs}ms` : `${s.at} · offline${s.error ? ` (${s.error})` : ""}`);
    return `<rect x="${x.toFixed(1)}" y="${P}" width="${hitW.toFixed(1)}" height="${uh}" fill="transparent"><title>${tip}</title></rect>`;
  }).join("");
  ...
  return `<div class="graph-wrap">
    ...
    <div class="graph-scale"><span>24h ago</span><span>12h ago</span><span>Now</span></div>
  </div>`;
}
...
function renderNodeCard(node) {
  ...
  const graph = renderPingGraph(node.history || []);
  ...
  return `<div class="node-card">
    ...
    <div class="node-stats">
      ...
      <div class="stat-chip"><span class="chip-label">Uptime 24h</span><span class="chip-val">${node.uptimePercent}%</span></div>
    ...
    <div class="timeline-wrap">
      ${timeline}
      <div class="timeline-meta"><span>24h ago</span><span class="uptime-pct">${node.uptimePercent}% uptime</span><span>Now</span></div>
    </div>
  </div>`;
}
...
function renderStatusPage(stats, nodes, nodeError, lastNodeUpdate) {
  ...
  const nodeCards = nodeError
    ...
    : (nodes.length ? nodes.map(renderNodeCard).join("") : '<div class="muted">No nodes found.</div>');
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Node cards */
    .nodes-section h2 { font-size: 16px; font-weight: 700; margin-bottom: 14px; }
    ...
    .ping-line { fill: none; stroke: var(--blue); stroke-width: 2; stroke-linejoin: round; stroke-linecap: round; filter: drop-shadow(0 0 4px rgba(79,142,255,.5)); }
    .ping-dot { fill: #c6d8ff; opacity: 0.3; }
  ...
  <script>setTimeout(() => location.reload(), ${SAMPLE_INTERVAL_MS});</script>
</head>
<body>
<div class="wrap">

  <div class="overall-banner ${overallClass}">
    <div class="overall-icon">${anyDown ? "⚠️" : "✅"}</div>
    <div class="overall-text">
      <h2>${overallLabel}</h2>
      <p>Last checked ${lastNodeUpdate ? new Date(lastNodeUpdate).toLocaleTimeString() : "—"} &nbsp;·&nbsp; Auto-refreshing every ${SAMPLE_INTERVAL_MS / 1000}s</p>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="stats-grid">
      <div class="stat-block">
        ...
        <div class="value ${stats.status === "ok" ? "value-ok" : "value-warn"}">${stats.status === "ok" ? "Online" : "Degraded"}</div>
      </div>
      <div class="stat-block">
        <div class="label">Monitor Uptime</div>
        <div class="value value-ok">${formatUptime(stats.uptimeSeconds)}</div>
      </div>
      <div class="stat-block">
        <div class="label">Nodes Online</div>
        <div class="value">${onlineNodes}<span style="font-size:14px;color:var(--muted)">/${nodes.length}</span></div>
      </div>
    </div>
    <div class="fleet-grid">
      <div class="fleet-pill"><span class="k">Operational</span><span class="v ok">${operationalNodes}</span></div>
      <div class="fleet-pill"><span class="k">Maintenance</span><span class="v warn">${maintenanceNodes}</span></div>
      <div class="fleet-pill"><span class="k">Offline</span><span class="v bad">${offlineNodes}</span></div>
      <div class="fleet-pill"><span class="k">Fleet Avg Uptime</span><span class="v">${fleetAvgUptime}%</span></div>
      <div class="fleet-pill"><span class="k">Fleet Avg Ping</span><span class="v">${fleetAvgPing}</span></div>
    </div>
    <div class="cluster-bar-wrap">
      <div class="cluster-bar-label"><span>Node availability</span><span>${nodeAvailabilityPct}%</span></div>
      <div class="cluster-bar-track"><div class="cluster-bar-fill" style="width:${nodeAvailabilityPct}%"></div></div>
    </div>
  ...
  <div class="card nodes-section">
    ...
    <div class="legend">
      ...
      <span class="legend-item"><span class="legend-dot" style="background:#8866ff;"></span>Mixed</span>
...
function buildNodePayload() {
  return [...nodeMonitor.nodes.values()].map(computeNodeView).sort((a, b) => String(a.name).localeCompare(String(b.name)));
}
